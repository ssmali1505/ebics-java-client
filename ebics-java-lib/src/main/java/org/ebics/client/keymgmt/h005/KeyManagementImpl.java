...
public class KeyManagementImpl extends KeyManagement {
   ...
   */
  @Override
  public String sendINI(String orderId) throws EbicsException, IOException {
   ...
   */
  @Override
  public String sendHIA(String orderId) throws IOException, EbicsException {
   ...
   */
  @Override
  public String sendHPB() throws IOException, GeneralSecurityException, EbicsException {
    ...
    System.out.println("session.getUser().getPartner().getBank().useCertificate():"+session.getUser().getPartner().getBank().useCertificate());
    if (session.getUser().getPartner().getBank().useCertificate())
    {
        ...
        Bank bank = (Bank)session.getUser().getPartner().getBank();
        if (bank.needsSave()) {           
        	session.getConfiguration().getSerializationManager().serialize(bank);
        }
    }
    else
    {
   ...
   */
  @Override
  public String lockAccess() throws IOException, EbicsException {
    HttpRequestSender			sender;
