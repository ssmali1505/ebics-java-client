...
/**
 * Handling of file transfers. Files can be transferred to and fetched from the
 * bank. Every transfer may be performed in a recoverable way. For convenience
 * and performance reasons there are also methods that do the whole transfer in
 * one method call. To use the recoverable transfer mode, you may set a working
 ...
 *
 * <p>
 * In the event of an upload transaction, a random symmetrical key is generated
 * in the customer system that is used exclusively within the framework of this
 * transaction both for encryption of the ES’s and for encryption of the order
 * data. This key is encrypted asymmetrically with the financial institution’s
 * public encryption key and is transmitted by the customer system to the bank
 * system during the initialization phase of the transaction.
 *
 * <p>
 * Analogously, in the case of a download transaction a random symmetrical key
 * is generated in the bank system that is used for encryption of the order data
 * that is to be downloaded and for encryption of the bank-technical signature
 * that has been provided by the financial institution. This key is
 * asymmetrically encrypted and is transmitted by the bank system to the
 * customer system during the initialization phase of the transaction. The
 * asymmetrical encryption takes place with the technical subscriber’s public
 * encryption key if the transaction’s EBICS messages are sent by a technical
 * subscriber. Otherwise the asymmetrical encryption takes place with the public
 * encryption key of the non-technical subscriber, i.e. the submitter of the
 * order.
 *
 * @author Hachani
 *
...
public class FileTransfer extends AbstractFileTransfer {
	 ...
	 */
	public String sendFile(byte[] content, EbicsUploadOrder ebicsUploadOrder) throws IOException, EbicsException {
		...
		InitializationResponseElement response = new InitializationResponseElement(sender.getResponseBody(), orderType,
		...
		while (state.hasNext()) {
			...
			String result = sendFileSegment(initializer.getContent(segmentNumber), segmentNumber, state.isLastSegment(),
					state.getTransactionId(), orderType);
			if (state.isLastSegment()) {
				return result;
			}
		}
		throw new EbicsException("Something went wrong.");
	}
	 ...
	 * @throws EbicsException
	 */
	protected String sendFileSegment(ContentFactory factory, int segmentNumber, boolean lastSegment,
			byte[] transactionId, EbicsAdminOrderType orderType) throws IOException, EbicsException {
		UploadTransferRequestElement uploader;
		HttpRequestSender sender;
		TransferResponseElement response;
		int httpCode;

		logger.info(Messages.getString("upload.segment", Constants.APPLICATION_BUNDLE_NAME, segmentNumber));
		uploader = new UploadTransferRequestElement(session, orderType, segmentNumber, lastSegment, transactionId,
				factory);
		...
		response.build();
		session.getConfiguration().getTraceManager().trace(response, session);
		return sender.getResponseAsString();
	}

	/**
	 * Fetches a file of the given order type from the bank. You may give an
	 * optional start and end date. This type of transfer will run until everything
	 * is processed. No transaction recovery is possible.
	 * 
	 ...
	 */
	public void fetchFile(EbicsDownloadOrder downloadOrder, File outputFile) throws IOException, EbicsException {
		HttpRequestSender sender;
		...
		Utils.checkHttpCode(httpCode);
		response = new DownloadInitializationResponseElement(sender.getResponseBody(), orderType,
				DefaultEbicsRootElement.generateName(orderType));
		...
		while (state.hasNext()) {
			...
			fetchFileSegment(orderType, segmentNumber, state.isLastSegment(), state.getTransactionId(), joiner);
		}

		try (FileOutputStream dest = new FileOutputStream(outputFile)) {
			joiner.writeTo(dest, response.getTransactionKey());
		}
		receipt = new ReceiptRequestElement(session, state.getTransactionId(),
				DefaultEbicsRootElement.generateName(orderType));
		receipt.build();
		receipt.validate();
	 ...
	 * @throws EbicsException server generated error
	 */
	protected void fetchFileSegment(EbicsAdminOrderType orderType, int segmentNumber, boolean lastSegment,
			byte[] transactionId, Joiner joiner) throws IOException, EbicsException {
		...
		sender = new HttpRequestSender(session);
		downloader = new DownloadTransferRequestElement(session, orderType, segmentNumber, lastSegment, transactionId);
		downloader.build();
		downloader.validate();
		...
		Utils.checkHttpCode(httpCode);
		response = new DownloadTransferResponseElement(sender.getResponseBody(), orderType,
				DefaultEbicsRootElement.generateName(orderType));
		response.build();
		session.getConfiguration().getTraceManager().trace(response, session);
		response.report();
